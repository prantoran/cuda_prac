        ...
        for (int i = -r; i <= r; i++) {
            for (int j = -r; j <= r; j++) {
                ...
                if (inRow >= 0 && inRow < height && inCol >= 0 && inCol < width) {
                    ...
                    Pvalue += M[inRow * width + inCol] * F[filterIndex];
                }
            }
        }
    ...
    if (outRow < height && outCol < width) {
        ...
        for (int i = -r; i <= r; i++) {
            for (int j = -r; j <= r; j++) {
                ...
                if (inRow >= 0 && inRow < height && inCol >= 0 && inCol < width) {
                    int filterIndex = (i + r) * (2 * r + 1) + (j + r);
                    Pvalue += M[inRow * width + inCol] * constFilter[filterIndex];
                }
            }
        }
        P[outRow * width + outCol] = Pvalue;
        ...
        for (int fRow = 0; fRow < 2 * FILTER_RADIUS + 1; fRow++) {
            for (int fCol = 0; fCol < 2 * FILTER_RADIUS + 1; fCol++) {
                ...
                if (sy >= 0 && sy < TILE_SIZE && sx >= 0 && sx < TILE_SIZE) {
                    ...
                    // then the ones that (we hope) already are in the l2 due the access of neighbouring
                    // threads, but first check if the values actually are within the matrix (the edge cases)
