// nvcc run_conv2d.cu conv2d_kernels.cu conv2d_functions.cu -DDEFINE_CONSTANT_MEMORY -o conv2d_prog && ./conv2d_prog 
#include <assert.h>
...
int main(int argc, char const* argv[]) {
    ...
    std::cout << "conv2d_with_constant_memory:\n";
    conv2d_with_constant_memory(M, gaussian_kernel, P, r, size, size);
    printMatrix(P, size, size);
    
    std::cout << "conv2d_with_tiled_convolution:\n";
    conv2d_with_tiled_convolution(M, gaussian_kernel, P, r, size, size);
    printMatrix(P, size, size);
    
    std::cout << "conv2d_with_tiled_convolution_with_l2_caching:\n";
