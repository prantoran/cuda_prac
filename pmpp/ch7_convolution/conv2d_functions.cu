...
void conv2d(float* M, float* F, float* P, int r, int height, int width) {
    ...
    error = cudaMalloc((void**)&d_F, (2*r + 1) * (2*r + 1) * sizeof(float));
    if (error != cudaSuccess) {
        std::cout << "cudaMalloc d_F failed: " << cudaGetErrorString(error) << std::endl;
        return;
    }
    ...
    conv2d_kernel<<<dimGrid, dimBloc>>>(d_M, d_F, d_P, r, height, width);
